...
#define LVDS_RESET_GPIO IMX_GPIO_NR(3,1)

static iomux_v3_cfg_t lvds_reset_pads[] = {
	IMX8MN_PAD_NAND_CE0_B__GPIO3_IO1 | MUX_PAD_CTRL(NO_PAD_CTRL),
};
...
#define GPIO_PAD_CFG_CTRL ( PAD_CTL_FSEL0 | PAD_CTL_ODE | PAD_CTL_PUE )
...
static iomux_v3_cfg_t const board_cfg[] = {
      ...
      IMX8MN_PAD_GPIO1_IO00__GPIO1_IO0 | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
...
/* IWG37M: Updating carrier board revision to device tree and Display selection */
int ft_board_setup (void *blob, struct bd_info *bd)
{
        ...
        fdt_setprop(blob,fdt_path_offset(blob, "/iwg37m_common_q7"), "pcb_rev", reg_pcb, sizeof(reg_pcb));
        fdt_setprop(blob,fdt_path_offset(blob, "/iwg37m_common_q7"), "bom_rev", reg_bom, sizeof(reg_bom));
        ...
        if (stmpe_i2c_reg != 0) /* Checking stmpe register, if stmpe_i2c_reg is 0 means stmpe found and otherwise not found */
        {
                ...
                value = fdt_setprop(blob,fdt_path_offset(blob, "/iwg37m_common_q7"), "carrier_pcb", pcb_stmp_reg, sizeof(pcb_stmp_reg));
                value = fdt_setprop(blob,fdt_path_offset(blob, "/iwg37m_common_q7"), "carrier_bom", bom_stmp_reg, sizeof(bom_stmp_reg));
        }
        else
        {
...
#ifndef CONFIG_TARGET_IMX8MN_IWG37M_Q7_UUU
	offset = fdt_setprop_string(blob, fdt_path_offset(blob, "/soc@0/bus@32c00000/usb@32e40000"), "dr_mode", "host");
	if (offset)
		return offset;
#endif
#endif
...
static iomux_v3_cfg_t lvds_backlight_pads[] = {
	IMX8MN_PAD_SPDIF_RX__GPIO5_IO4 | MUX_PAD_CTRL(NO_PAD_CTRL),
	IMX8MN_PAD_SPDIF_EXT_CLK__GPIO5_IO5 | MUX_PAD_CTRL(NO_PAD_CTRL),
};
...
int board_late_init(void)
{
	...
	env_set("board_name", "iW-RainboW-G37M-i.MX8M Nano Qseven SOM");
