...
static void wifi_pwr_seq(void)
{
	...
	dm_gpio_set_dir_flags(&pdn_en_desc, GPIOD_IS_OUT );
	...
	dm_gpio_set_dir_flags(&core_en_desc, GPIOD_IS_OUT);

	dm_gpio_set_value(&pdn_en_desc, 0);
	dm_gpio_set_value(&core_en_desc, 0);
}
...
static iomux_cfg_t hub_pwr_pads[] = {
	SC_P_HDMI_TX0_TS_SDA | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),
};
...
static void usb_hub_pwr_enable(void)
{
	...
	ret = dm_gpio_lookup_name("GPIO2_3", &hub_reset_desc);
	if (ret) {
...
static iomux_cfg_t lvds_bkt_pads[] = {
        SC_P_LVDS0_I2C1_SDA | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CTRL),
};

void lvds_backlight_disable(void)
{
        imx8_iomux_setup_multiple_pads(lvds_bkt_pads, ARRAY_SIZE(lvds_bkt_pads));

        struct gpio_desc lvds_power_desc;
        int ret;

        ret = dm_gpio_lookup_name("GPIO1_9", &lvds_power_desc);
        if (ret) {
                printf("%s lookup GPIO1_9 failed ret = %d\n", __func__, ret);
                return;
        }

        ret = dm_gpio_request(&lvds_power_desc, "lvds_bcklight_gpio");
        if (ret) {
                printf("%s request lvds_power gpio failed ret = %d\n", __func__, ret);
                return;
        }

        dm_gpio_set_dir_flags(&lvds_power_desc, GPIOD_IS_OUT);
        dm_gpio_set_value(&lvds_power_desc, 0);
}

#define BCONFIG_0 IMX_GPIO_NR(2, 9)
#define BCONFIG_1 IMX_GPIO_NR(4, 22)
#define BCONFIG_2 IMX_GPIO_NR(4, 11)
#define BCONFIG_3 IMX_GPIO_NR(3, 24)
...
static iomux_cfg_t board_cfg[] = {
	SC_P_ESAI1_TX1 | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
	SC_P_QSPI1A_DQS | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
	SC_P_USDHC2_WP | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
	SC_P_ADC_IN6 | MUX_MODE_ALT(3) | MUX_PAD_CTRL(GPIO_PAD_CFG_CTRL),
...
static void print_board_info(void)
{
	...
	printf ("\tSOM Version     : iW-PRGMZ-AP-01-R%x.%x\n",pcb_rev+1,bom_rev);
...
void iwg27m_fdt_update(void *fdt)
{
	...
	/* CMA: change the CMA memory alloc-range for 4GB LPDDR4 supported board */
