...
static iomux_v3_cfg_t const reset_pads[] = {
        IMX8MM_PAD_SAI3_MCLK_GPIO5_IO2  | MUX_PAD_CTRL(RESET_PAD_CTRL),
};
...
static iomux_v3_cfg_t const board_cfg[] = {
	IMX8MM_PAD_GPIO1_IO09_GPIO1_IO9 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SAI3_TXD_GPIO5_IO1 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SAI2_TXC_GPIO4_IO25 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SAI2_TXD0_GPIO4_IO26 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SAI2_MCLK_GPIO4_IO27 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SD2_RESET_B_GPIO2_IO19 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
	IMX8MM_PAD_SD2_WP_GPIO2_IO20 | MUX_PAD_CTRL(CONFG_GPIO_PAD_CTRL),
};
...
int ft_board_setup(void *blob, struct bd_info *bd) 
{
	...
	fdt_setprop(blob,fdt_path_offset(blob, "/iwg34m_common"), "pcb_rev", reg_pcb, sizeof(reg_pcb));
	fdt_setprop(blob,fdt_path_offset(blob, "/iwg34m_common"), "bom_rev", reg_bom, sizeof(reg_bom));
	...
	{
		stmp_reg[0] = cpu_to_fdt32(stmpe_i2c_reg);
		value = fdt_setprop(blob,fdt_path_offset(blob, "/iwg34m_common"), "carrier_bom", stmp_reg, sizeof(stmp_reg));
	}
	else
		stmp_reg[0] = cpu_to_fdt32(carrier_revision);
	value = fdt_setprop(blob,fdt_path_offset(blob, "/iwg34m_common"), "carrier_bom", stmp_reg, sizeof(stmp_reg));
...
/* Support for 1GB LPDDR4 Board */
	...
	if (!strcmp("imx8mm-iwg34m-rpmsg.dtb", env_get("fdtfile"))) {
...
board_late_mmc_env_init();
	...
	if (IS_ENABLED(CONFIG_ENV_VARS_UBOOT_RUNTIME_CONFIG)) {
		env_set("board_name", "iW-RainboW-G34M-i.MX8M Mini SODIMM SOM");
		env_set("board_rev", "iW-PRFYZ-AP-01-R1.X");
	}
