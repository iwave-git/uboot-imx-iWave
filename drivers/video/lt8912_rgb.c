...
static const struct display_timing default_timing_hdmi = {
        ...
        .vback_porch.typ        = 36,
        .vsync_len.typ          = 5,
        .flags                  = DISPLAY_FLAGS_HSYNC_LOW | DISPLAY_FLAGS_VSYNC_LOW | DISPLAY_FLAGS_PIXDATA_POSEDGE | DISPLAY_FLAGS_DE_HIGH,
};
...
void lvds_rgb_init(struct lt8912b_priv *priv, struct udevice *dev)
{
        lt8912b_i2c_reg_write(priv->cec_dev,0x10,0x01);
        lt8912b_i2c_reg_write(priv->cec_dev,0x11,0x04);
        lt8912b_i2c_reg_write(priv->cec_dev,0x18,0x80);
        lt8912b_i2c_reg_write(priv->cec_dev,0x19,0x02);
        lt8912b_i2c_reg_write(priv->cec_dev,0x1c,0x20);
        lt8912b_i2c_reg_write(priv->cec_dev,0x1d,0x03);
        lt8912b_i2c_reg_write(priv->cec_dev,0x2f,0x0c);
        lt8912b_i2c_reg_write(priv->cec_dev,0x34,0x20);
        lt8912b_i2c_reg_write(priv->cec_dev,0x35,0x04);
        lt8912b_i2c_reg_write(priv->cec_dev,0x36,0x0d);
        lt8912b_i2c_reg_write(priv->cec_dev,0x37,0x02);
        lt8912b_i2c_reg_write(priv->cec_dev,0x38,0x21);
        lt8912b_i2c_reg_write(priv->cec_dev,0x39,0x00);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3a,0x0a);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3b,0x00);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3c,0x58);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3d,0x00);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3e,0x28);
        lt8912b_i2c_reg_write(priv->cec_dev,0x3f,0x00);

	lt8912b_i2c_reg_write(dev,0x9c,0x73);
	lt8912b_i2c_reg_write(dev,0x9d,0x45);
	lt8912b_i2c_reg_write(dev,0x9e,0xee);
	lt8912b_i2c_reg_write(dev,0x9f,0x02);
...
void hdmi_rgb_init(struct lt8912b_priv *priv, struct udevice *dev)
{
	/* DigitalClockEn */
	...
	/* TxAnalog */
	lt8912b_i2c_reg_write(dev, 0x31, 0xa1);
	lt8912b_i2c_reg_write(dev, 0x32, 0xa1);
...
static int lt8912b_enable(struct udevice *dev)
{
	...
	{
		/* Enable Backlight */
		dm_gpio_set_value(&priv->bk_lgt, false);
		mdelay(100);
		dm_gpio_set_value(&priv->bk_lgt, true);
		
		//second reset for lvds
        	mdelay(30);
       		dm_gpio_set_value(&priv->reset_lvds, 1);
        	mdelay(120);
        	dm_gpio_set_value(&priv->reset_lvds, 0);
	}
	...
	if(priv->sink_is_hdmi)
		hdmi_rgb_init(priv, dev);
	else
		lvds_rgb_init(priv, dev);
	return 0;
}
...
static int lt8912b_rgb_probe(struct udevice *dev)
{
        struct lt8912b_priv *priv = dev_get_priv(dev);
	int ret;
	debug("%s\n", __func__);
	...
	if (!strcmp("lvds", env_get("disp"))) {
		priv->format = MIPI_DSI_FMT_RGB888;
               //lvds reset changes
                gpio_request_by_name(dev, "reset-gpio", 0, &priv->reset_lvds,
                            GPIOD_IS_OUT);
                if (ret)
                        return ret;
	}
	...
	if (ret) {
                printf("Warning: cannot get backlight gpio GPIO\n");
                if (ret != -ENOENT)
                        return ret;
        }
...
static int lt8912b_remove(struct udevice *dev)
{
	...
	gpio_request_by_name(dev, "backlight-en-gpio", 0, &priv->bk_lgt_en,
			    GPIOD_IS_OUT);

	dm_gpio_set_value(&priv->bk_lgt_en, false);
	dm_gpio_set_value(&priv->bk_lgt, false);

	if (!strcmp("lvds", env_get("disp")))
		lt8912b_i2c_reg_write(dev, 0x02, 0x00);
	mdelay(120);
...
U_BOOT_DRIVER(lt8912b_rgb_mipi2hdmi) = {
	.name			  = "lt8912b_rgb_mipi2hdmi",
	.id			  = UCLASS_PANEL,
	.of_match		  = lt8912b_ids,
	.ops			  = &lt8912b_ops,
	.probe			  = lt8912b_rgb_probe,
