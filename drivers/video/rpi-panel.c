...
static int rpi_enable(struct udevice *dev)
{
	...
	int ret, i;
	struct udevice *i2c_dev = priv->i2c_dev;

	dsi->mode_flags |= MIPI_DSI_MODE_LPM;
	rpi_touchscreen_i2c_write_eic(i2c_dev, REG_POWERON, 1);
        /* Wait for nPWRDWN to go low to indicate poweron is done. */
        for (i = 0; i < 100; i++) {
                if (rpi_touchscreen_i2c_read(i2c_dev, REG_PORTB) & 1)
                        break;
        	}
	...
	mdelay(100);
	rpi_touchscreen_i2c_write_eic(i2c_dev, REG_PWM, 255);
	rpi_touchscreen_i2c_write_eic(i2c_dev, REG_PORTA, BIT(2));
	return 0;
}
...
U_BOOT_DRIVER(rpi_panel) = {
	...
	.priv_auto_alloc_size	= sizeof(struct rpi_panel_priv),
};
